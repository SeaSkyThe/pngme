...
impl Png {
    ...
    fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk, String> {
        ...
        if let Some(index) = self.chunks.iter().position(|x| x.chunk_type == chunk_type) {
            return Ok(self.chunks.remove(index));
        }
        Err(format!("Error removing first ocurrence of {chunk_type}."))
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        if signature != Png::STANDARD_HEADER {
            return Err("Invalid signature header for PNG.".to_string());
        }
        let chunks_bytes = &value[8..];
        ...
        // [u8;cs] -> chunk data
        // [u8;4] -> crc
        // So, to get one chunk and transform it, we have to read the cs
        // in the first 4 bytes, then get value[last_chunk_end..4+4+cs+4]

        // Probably very non-rusty of me doing it that way
        let size = chunks_bytes.len();
        let mut curr_chunk_start = 0;
        let mut chunks: Vec<Chunk> = vec![];
        while curr_chunk_start < size {
            let curr_chunk_size_bytes: [u8; 4] = chunks_bytes
                [curr_chunk_start..curr_chunk_start + 4]
                .try_into()
                .unwrap();
            let curr_chunk_size = <u32>::from_be_bytes(curr_chunk_size_bytes);

            let curr_chunk_end = curr_chunk_start + 4 + 4 + 4 + curr_chunk_size as usize;
            let curr_chunk_bytes = &chunks_bytes[curr_chunk_start..curr_chunk_end];
